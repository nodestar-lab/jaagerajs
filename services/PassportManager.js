    ...
    initialize() {
        passport.use(
            new LocalStrategy({
                ...
                async (username, password, done) => {
                    ...
                    if (result._id) {
                        logger.info(`--LocalStrategy logged in user is ${result.username}`);
        ...
        passport.use(
            new JWTStrategy({
                ...
                (jwtPayload, cb) => {
                    ...
                    return this.jr.DBManager.db.user.findOne(jwtPayload.id)
                        .then(user => {
                            return cb(null, user);
                        })
                        .catch(err => {
        ...
        passport.use(
            new GoogleStrategy({
                ...
                this.jr.DBManager.db.user.findOne({
                    googleId: profile.id
                }).then((currentUser) => {
                    if (currentUser) {
